...
1. Parse your own command name to extract the module ID. This command is `start-1-5`, so the module ID is `1.5`.

2. Read `course-structure.json`. Find the module where `"id": "1.5"`. Load the teaching script at the `path` field for that module.
